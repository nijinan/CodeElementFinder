...
import cn.edu.pku.sei.tsr.APIfinder.codeparser.code.entity.JavaBaseInfo;
...
import cn.edu.pku.sei.tsr.APIfinder.utils.ObjectIO;
...
public class CodeTermsParser {
		...
		if (term.isLeaf() && term.isMethod()){
			...
			if (term.getRelevantCodeElement() == null) {
				if (term.getSimilarMethodCodeElementList().size() > 0){
					JavaMethodInfo ans = (JavaMethodInfo) term.getSimilarMethodCodeElementList().keySet().toArray()[0];
					
					for (Object t : term.getSimilarMethodCodeElementList().keySet().toArray()){
						if (((JavaMethodInfo)t).getBelongType().getFullyQualifiedName().split("\\.").length >
						ans.getBelongType().getFullyQualifiedName().split("\\.").length){
							ans = (JavaMethodInfo)t;
						}
					}
					term.setRelevantCodeElement((JavaBaseInfo)ans);
				}else
	...
	private void parseTypeContextFilter(CodeLikeTermInfo term){
		...
		boolean flag = false;
		if (term.getContent().contains("QueryParser")){
			System.out.println("");
		}
		if (term.isLeaf() && term.isType()){
			...
			if (term.getRelevantCodeElement() == null) {
				if (term.getSimilarTypeCodeElementList().size() > 0){
					JavaTypeInfo ans = (JavaTypeInfo) term.getSimilarTypeCodeElementList().keySet().toArray()[0];
					
					for (Object t : term.getSimilarTypeCodeElementList().keySet().toArray()){
						if (((JavaTypeInfo)t).getFullyQualifiedName().split("\\.").length >
						ans.getFullyQualifiedName().split("\\.").length){
							ans = (JavaTypeInfo)t;
						}
					}
					term.setRelevantCodeElement((JavaBaseInfo)ans);
